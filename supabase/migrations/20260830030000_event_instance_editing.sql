create or replace function private.prevent_event_series_change()
returns trigger
language plpgsql
...
begin
  if new.series_id is distinct from old.series_id then
    raise exception using
      errcode = '42501',
      message = 'event series link cannot be changed';
  end if;
  return new;
end;
$$;

revoke all on function private.prevent_event_series_change() from public;

create trigger events_series_immutable
before update on public.events
for each row execute function private.prevent_event_series_change();
